...
import { OBJECT_TEXTURE, ObjectType, TERRAIN_TEXTURE } from './TileTypes';
...
export class WorldRenderer {
  constructor(scene: Phaser.Scene, map: WorldMap) {
    ...
    const staticLayer = this.bakeStaticObjects(scene, map, worldWidth, worldHeight);
  ...
  private bakeTerrain(scene: Phaser.Scene, map: WorldMap, worldWidth: number, worldHeight: number): void {
    ...
    for (let tileY = 0; tileY < map.height; tileY++) {
      for (let tileX = 0; tileX < map.width; tileX++) {
        ...
        renderTexture.draw(TERRAIN_TEXTURE[terrain], tileX * TILE_SIZE, tileY * TILE_SIZE);
      }
    }

    renderTexture.saveTexture('world_terrain');
    renderTexture.destroy();
    scene.add.image(0, 0, 'world_terrain').setOrigin(0, 0);
  }
  ...
  private bakeStaticObjects(
    ...
    renderTexture.destroy();
    return scene.add.image(0, 0, 'world_static_objects').setOrigin(0, 0);
  }
}
